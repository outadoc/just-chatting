...
class ChannelViewerListDataDeserializer : JsonDeserializer<ChannelViewerListDataResponse> {
    ...
    override fun deserialize(
        ...
        val dataJson = json.asJsonObject.getAsJsonObject("data").getAsJsonObject("channel")
            .getAsJsonObject("chatters")
        dataJson.getAsJsonArray("broadcasters").forEach {
            val obj = it.asJsonObject
            if (!(obj.get("login").isJsonNull)) {
                broadcasters.add(obj.getAsJsonPrimitive("login").asString)
            }
        }
        dataJson.getAsJsonArray("moderators").forEach {
            val obj = it.asJsonObject
            if (!(obj.get("login").isJsonNull)) {
                moderators.add(obj.getAsJsonPrimitive("login").asString)
            }
        }
        dataJson.getAsJsonArray("vips").forEach {
            val obj = it.asJsonObject
            if (!(obj.get("login").isJsonNull)) {
                vips.add(obj.getAsJsonPrimitive("login").asString)
            }
        }
        dataJson.getAsJsonArray("viewers").forEach {
            val obj = it.asJsonObject
            if (!(obj.get("login").isJsonNull)) {
                viewers.add(obj.getAsJsonPrimitive("login").asString)
            }
        }
        val count = if (!(dataJson.get("count").isJsonNull)) {
            dataJson.getAsJsonPrimitive("count").asInt
        } else null
        return ChannelViewerListDataResponse(
            ChannelViewerList(
                broadcasters,
                moderators,
                vips,
