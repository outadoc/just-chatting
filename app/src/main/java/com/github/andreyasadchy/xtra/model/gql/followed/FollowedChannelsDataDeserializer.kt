...
class FollowedChannelsDataDeserializer : JsonDeserializer<FollowedChannelsDataResponse> {
    ...
    override fun deserialize(
        ...
        val dataJson = json.asJsonObject.getAsJsonObject("data").getAsJsonObject("user")
            .getAsJsonObject("follows").getAsJsonArray("edges")
        dataJson.forEach {
            cursor =
            ...
            data.add(
                Follow(
                    to_id = if (!(obj.get("id").isJsonNull)) {
                        obj.getAsJsonPrimitive("id").asString
                    } else null,
                    to_login = if (!(obj.get("login").isJsonNull)) {
                        obj.getAsJsonPrimitive("login").asString
                    } else null,
                    to_name = if (!(obj.get("displayName").isJsonNull)) {
