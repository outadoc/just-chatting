    ...
    override fun deserialize(
        ...
        dataJson.forEach {
            cursor = if (!json.asJsonObject.getAsJsonObject("data").getAsJsonObject("searchFor")
                .getAsJsonObject("channels").get("cursor").isJsonNull
            ) json.asJsonObject.getAsJsonObject("data").getAsJsonObject("searchFor")
                .getAsJsonObject("channels").getAsJsonPrimitive("cursor").asString else null
            ...
            data.add(
                ChannelSearch(
                    ...
                    broadcaster_login = if (!(obj.get("login").isJsonNull)) {
                        obj.getAsJsonPrimitive("login").asString
                    } else null,
                    display_name = if (!(obj.get("displayName").isJsonNull)) {
                    ...
                    followers_count = if (!(obj.get("followers").isJsonNull)) {
                        obj.getAsJsonObject("followers").getAsJsonPrimitive("totalCount").asInt
                    } else null,
                )
            )
        }
