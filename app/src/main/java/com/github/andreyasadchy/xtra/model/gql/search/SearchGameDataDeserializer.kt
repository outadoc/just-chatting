...
class SearchGameDataDeserializer : JsonDeserializer<SearchGameDataResponse> {
    ...
    ): SearchGameDataResponse {
        val data = mutableListOf<Game>()
        var cursor: String? = null
        val dataJson = json.asJsonObject.getAsJsonObject("data").getAsJsonObject("searchFor")
            .getAsJsonObject("games").getAsJsonArray("edges")
        dataJson.forEach {
            cursor = if (!json.asJsonObject.getAsJsonObject("data").getAsJsonObject("searchFor")
                .getAsJsonObject("games").get("cursor").isJsonNull
            ) json.asJsonObject.getAsJsonObject("data").getAsJsonObject("searchFor")
                .getAsJsonObject("games").getAsJsonPrimitive("cursor").asString else null
            ...
            data.add(
                Game(
                    ...
                    name = if (!(obj.get("displayName").isJsonNull)) {
                        obj.getAsJsonPrimitive("displayName").asString
                    } else null,
                    box_art_url = if (!(obj.get("boxArtURL").isJsonNull)) {
                        obj.getAsJsonPrimitive("boxArtURL").asString
                    } else null,
                    viewersCount = if (!(obj.get("viewersCount").isJsonNull)) {
                        obj.getAsJsonPrimitive("viewersCount").asInt
                    } else 0, // returns null if 0
