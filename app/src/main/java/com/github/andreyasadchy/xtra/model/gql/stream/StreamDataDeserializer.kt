...
class StreamDataDeserializer : JsonDeserializer<StreamDataResponse> {
    ...
    override fun deserialize(
        ...
        val dataJson = json.asJsonObject.getAsJsonObject("data").getAsJsonObject("streams")
        ...
        dataJson.forEach {
            ...
            data.add(
                Stream(
                    ...
                    user_id = if (!(obj.get("broadcaster").isJsonNull)) {
                        obj.getAsJsonObject("broadcaster").getAsJsonPrimitive("id").asString
                    } else null,
                    user_login = if (!(obj.get("broadcaster").isJsonNull)) {
                        obj.getAsJsonObject("broadcaster").getAsJsonPrimitive("login").asString
                    } else null,
                    user_name = if (!(obj.get("broadcaster").isJsonNull)) {
                    ...
                    type = if (!(obj.get("type").isJsonNull)) {
                        obj.getAsJsonPrimitive("type").asString
                    } else null,
                    title = if (!(obj.get("title").isJsonNull)) {
                        obj.getAsJsonPrimitive("title").asString
                    } else null,
                    viewer_count = if (!(obj.get("viewersCount").isJsonNull)) {
                        obj.getAsJsonPrimitive("viewersCount").asInt
                    } else 0,
                    thumbnail_url = if (!(obj.get("previewImageURL").isJsonNull)) {
                        obj.getAsJsonPrimitive("previewImageURL").asString
                    } else null,
