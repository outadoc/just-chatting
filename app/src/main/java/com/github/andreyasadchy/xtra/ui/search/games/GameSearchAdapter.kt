...
class GameSearchAdapter(
    ...
    override fun bind(item: Game, view: View) {
        with(view) {
            ...
            gameImage.loadImage(fragment, TwitchApiHelper.getTemplateUrl(item.box_art_url, "game", if (context.prefs().getBoolean(C.API_USEHELIX, true) && context.prefs().getString(C.USERNAME, "") != "" && context.prefs().getBoolean(C.API_USEHELIX_GAMES, false)) "3" else "4"))
