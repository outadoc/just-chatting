...
internal fun PubSubPollMessage.Data.Poll.map(): Poll = Poll(
    pollId = pollId,
    status = status.map(),
    title = title,
...
private fun PubSubPollMessage.Data.Poll.Status.map(): Poll.Status = when (this) {
...
private fun PubSubPollMessage.Data.Poll.Choice.map(): Poll.Choice = Poll.Choice(
    choiceId = choiceId,
    title = title,
    votes = votes.map(),
    totalVoters = totalVoters,
...
private fun PubSubPollMessage.Data.Poll.Votes.map(): Poll.Votes = Poll.Votes(
    total = total,
    bits = bits,
