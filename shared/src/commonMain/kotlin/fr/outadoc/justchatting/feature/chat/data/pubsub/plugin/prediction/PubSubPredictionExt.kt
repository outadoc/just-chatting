...
internal fun PubSubPredictionMessage.Outcome.map(): Prediction.Outcome = Prediction.Outcome(
    id = id,
    title = title,
    color =
    when (color) {
...
internal fun PubSubPredictionMessage.map(): Prediction = when (val prediction = data.event) {
