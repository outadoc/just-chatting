...
internal class TwitchClient(httpClient: HttpClient) {
    ...
    suspend fun getStreamsByUserId(ids: List<String>): Result<StreamsResponse> = runCatching {
        client.get {
            url {
                path("streams")
                ids.forEach { id ->
    ...
    suspend fun getStreamsByUserLogin(logins: List<String>): Result<StreamsResponse> = runCatching {
        client.get {
            url {
                path("streams")
                logins.forEach { id ->
    ...
    suspend fun getFollowedStreams(
        ...
        limit: Int,
        after: String?,
    ): Result<StreamsResponse> = runCatching {
        client.get {
            url {
    ...
    suspend fun getUsersById(ids: List<String>): Result<UsersResponse> = runCatching {
        client.get {
            url {
                path("users")
                ids.forEach { id ->
    ...
    suspend fun getUsersByLogin(logins: List<String>): Result<UsersResponse> = runCatching {
        client.get {
            url {
                path("users")
    ...
    suspend fun searchChannels(
        ...
        limit: Int,
        after: String?,
    ): Result<ChannelSearchResponse> = runCatching {
        client.get {
            url {
    ...
    suspend fun getEmotesFromSet(setIds: List<String>): Result<EmoteSetResponse> = runCatching {
        client.get {
            url {
    ...
    suspend fun getCheerEmotes(userId: String?): Result<CheerEmotesResponse> = runCatching {
        client.get {
            url {
    ...
    suspend fun getGlobalBadges(): Result<TwitchBadgesResponse> = runCatching {
        client.get { url { path("chat/badges/global") } }.body()
    }

    suspend fun getChannelBadges(channelId: String): Result<TwitchBadgesResponse> = runCatching {
        client.get {
            url {
                path("chat/badges")
    ...
    suspend fun getChannelSchedule(
        ...
        limit: Int,
        after: String?,
    ): Result<ChannelScheduleResponse> = runCatching {
