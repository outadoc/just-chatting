...
internal class TwitchIrcCommandParserTest {
    ...
    @Test
    fun `Parse action PRIVMSG`() =
        test {
            input {
                "@badge-info=subscriber/11;badges=moderator/1,subscriber/3009;color=#8A2BE2;display-name=pepitipepibot;emotes=;first-msg=0;flags=;id=7485e9c7-962f-4fe9-af27-3b625699f7e1;mod=1;returning-chatter=0;room-id=402890635;subscriber=1;tmi-sent-ts=1657298849487;turbo=0;user-id=651859616;user-type=mod :pepitipepibot!pepitipepibot@pepitipepibot.tmi.twitch.tv PRIVMSG #pelerine :\u0001ACTION ⭐️ RE-SUB ⭐️ Amphirae (+4) ⭐️\u0001"
            }
            expected {
                ChatEvent.Message.ChatMessage(
    ...
    @Test
    fun `Parse mass sub gift USERNOTICE`() =
        test {
            input {
    ...
    @Test
    fun `Parse pay forward USERNOTICE`() =
        test {
            input {
                "@badge-info=;badges=bits/100;color=#D2691E;display-name=FichierPDF;emotes=;flags=;id=5afe1c23-a512-4122-b80f-85000ac130bd;login=fichierpdf;mod=0;msg-id=communitypayforward;msg-param-prior-gifter-anonymous=false;msg-param-prior-gifter-display-name=Hipairion;msg-param-prior-gifter-id=590747111;msg-param-prior-gifter-user-name=hipairion;room-id=135468063;subscriber=1;system-msg=FichierPDF\\sis\\spaying\\sforward\\sthe\\sGift\\sthey\\sgot\\sfrom\\sHipairion\\sto\\sthe\\scommunity!;tmi-sent-ts=1682099586016;user-id=31950715;user-type= :tmi.twitch.tv USERNOTICE #antoinedaniel"
            }
            expected {
    ...
    @Test
    fun `Parse announcement USERNOTICE`() =
        test {
            input {
    ...
    @Test
    fun `Parse shared chat announcement USERNOTICE`() =
        test {
            input {
    ...
    @Test
    fun `Parse moderator anniversary USERNOTICE`() =
        test {
            input {
    ...
    @Test
    fun `Parse watch streak USERNOTICE`() =
        test {
            input {
    ...
    @Test
    fun `Parse watch streak USERNOTICE with unprefixed trailing message`() =
        test {
            input {
    ...
    @Test
    fun `Parse viewermilestone USERNOTICE with unknown category`() =
        test {
            input {
    ...
    @Test
    fun `Parse CLEARCHAT permanent ban message`() =
        test {
    ...
    @Test
    fun `Parse CLEARCHAT temporary ban message`() =
        test {
            input {
    ...
    @Test
    fun `Parse CLEARCHAT global clear message`() =
        test {
    ...
    @Test
    fun `Parse USERSTATE message`() =
        test {
            input {
                "@badge-info=;badges=;color=#F85E10;display-name=outadoc;emote-sets=0,19194,553791,773027,300374282,380519965,477339272,494046698,537206155,592920959,610186276;id=e39d4a41-a7c8-4a24-845d-3e1c26aaf63f;mod=0;subscriber=0;user-type= :tmi.twitch.tv USERSTATE #mistermv"
            }
            expected {
    ...
    @Test
    fun `Parse highlighted message`() =
        test {
            input {
                "@badge-info=subscriber/21;badges=subscriber/18,premium/1;color=#FF0000;display-name=FlorianPremier;emotes=;first-msg=0;flags=;id=68d0ad7e-7743-4b51-b8fd-3d995eb17fd5;mod=0;msg-id=highlighted-message;returning-chatter=0;room-id=135468063;subscriber=1;tmi-sent-ts=1661452379625;turbo=0;user-id=137824138;user-type= :florianpremier!florianpremier@florianpremier.tmi.twitch.tv PRIVMSG #antoinedaniel :vive l'argent"
            }
            expected {
    ...
    data class Assertion(
        val input: String,
        val expected: ChatEvent,
    ) {
        class Builder {
            private var input: String? = null
            private var expected: ChatEvent? = null

            fun input(block: () -> String) {
                input = block()
            }

            fun expected(block: () -> ChatEvent) {
                expected = block()
            }
    ...
    private fun test(block: Assertion.Builder.() -> Unit) {
        ...
        assertEquals(
            ...
            actual = parser.parse(assertion.input),
        )
    }
}
